"""Manifest-driven MCP Streamable HTTP policy core for APIM Consumption."""
...
def is_model_visible_parameter(parameter: dict) -> bool:
    return not (
...
def hidden_parameters(tool: ToolDefinition) -> list[dict]:
    return [
        parameter for parameter in operation_parameters(tool)
        if not is_model_visible_parameter(parameter)
    ]
...
def sample_tool_calls(tool: ToolDefinition) -> list[tuple[dict, object, bool]]:
    """Every model-reachable x-mock branch -> MCP args and expected result."""
    calls = []
    hidden_names = {parameter["name"] for parameter in hidden_parameters(tool)}
    rules = [
        rule for rule in mock_rules(tool)
        if not rule.get("when")
        or rule["when"].get("param") not in hidden_names
    ]
...
def initialize_body(server_name: str) -> str:
 ...
 var version=!string.IsNullOrEmpty(requested)&&supported.Contains(requested)?requested:"2025-11-25";
 var result=new JObject(new JProperty("protocolVersion",version),new JProperty("capabilities",new JObject(new JProperty("tools",new JObject(new JProperty("listChanged",false))))),new JProperty("serverInfo",new JObject(new JProperty("name","%s"),new JProperty("version","%s"))));
 %s
}""" % (csharp_string(server_name), csharp_string(__version__),
          rpc_envelope("result"))
...
def mock_rules(tool: ToolDefinition) -> list[dict]:
    ...
    for index, rule in enumerate(rules):
        ...
        if "when" not in rule:
            ...
            if (not is_model_visible_parameter(parameters[name])
                    and operators[0] != "missing"):
                raise PolicyBuildError(
...
def tool_call_branch(tool: ToolDefinition, first: bool) -> list[str]:
    ...
    for parameter in hidden_parameters(tool):
        name = csharp_string(parameter["name"])
        lines.append(
            f' args["{name}"]=System.Guid.NewGuid().ToString();')
...
def add_json_response(parent, body: str, status: int = 200):
    ...
    ET.SubElement(response, "set-status", {
        "code": str(status),
        "reason": {202: "Accepted", 400: "Bad Request"}.get(status, "OK"),
    })
    if status != 202:
...
def build_policy(server_name: str, tools: list[ToolDefinition]) -> str:
    ...
    ET.SubElement(inbound, "set-variable", {
        "name": "rpc",
        "value": ("@{try{return context.Request.Body.As<JObject>"
                  "(preserveContent:true);}catch{return null;}}")})
    parse_guard = ET.SubElement(inbound, "choose")
    parse_error = ET.SubElement(parse_guard, "when", {
        "condition": '@(context.Variables.GetValueOrDefault<JObject>("rpc") == null)'})
    add_json_response(parse_error, compact({
        "jsonrpc": "2.0", "id": None,
